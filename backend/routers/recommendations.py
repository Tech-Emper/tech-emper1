...
async def get_optional_user(authorization: str = Header(None)):
    """Returns user payload if token is valid, otherwise returns None (no 401)."""
    if not authorization or not authorization.startswith("Bearer "):
        return None
    token = authorization.split(" ")[1]
    if token in ("null", "undefined", ""):
        return None
    return decode_access_token(token)  # Returns None if expired/invalid

@router.post("/recommend")
def get_recommendation(data: UserData, user_payload = Depends(get_optional_user), db: Session = Depends(get_db)):
    try:
        from logic import calculate_recommendation
        email = user_payload.get("sub") if user_payload else None
        
        # Only persist user data if they are authenticated
        if email:
        ...
        # Save the recommendation only if authenticated
        if email and 'user' in dir():
